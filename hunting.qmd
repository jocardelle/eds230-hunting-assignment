...
Looking at the plot results from this parameter grid, we can see that a hunting rate of 0.01 is the most stable, combined with a minimum prey population of 50.
...
Based on our parameter grid analysis above, we identified the most stable hunting scenario as `rhunt` = 0.01 and `prey_min` = 50. With these parameters both populations remained above the stability threshold the most through the 100 days and had sustained oscillations. We'll now use these parameters to calculate our stable predator and prey populations. 

```{r}
# Set parameter list
pars <- data.frame(rprey = 0.95, alpha = 0.01, eff = 0.6, pmort = 0.4, K = 2000, rhunt = 0.01, prey_min = 50)
