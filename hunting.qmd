...
```{r}
source(here("lotvhuntmod_hunt.R"))
```
...
days_short <- seq(1, 500, by = 1)

results <- scenarios %>%
  mutate(
    output = map2(rhunt, prey_min, function(rh, pm) {
      pars <- c(base_params, rhunt = rh, prey_min = pm)
      as.data.frame(ode(y = currpop, times = days_short, func = lotvhunt, parms = pars, method = "radau"))
    })
  )
